...
class AddWaiting extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:add-waiting {type}';
    ...
    public function handle()
    {
        ...
        if($type === 'simcard') {
            ...
            $simcards = RakutenDataSim::where('status', 'active')->get();
            $this->info("Simcards count ====> ". $simcards->count());

            $class_name = RakutenDataSim::class;
            $date = Carbon::now()->format('Y-m-d');
            ...
            \App\Models\mobile\WaitingBillingGenerateSim::insert($data);
            return 0;
        } elseif($type === 'rakuten_call') {
            $simcards = RakutenCallSim::where(function($query) {
                $query->where('status', 'active')->where('activation_date', '<', Carbon::now()->startOfMonth());
            ...
            \App\Models\mobile\WaitingBillingGenerateSim::insert($data);
            return 0;
        } elseif($type === 'simcard_b') {
            $simcards = SimcardB::where(function($query){
                $query->where('status', 'active')->where('activation_date', '<', Carbon::now()->startOfMonth())->orWhere(function($query) {
                $query->where('status', 'deactivate')->whereMonth('deactivation_date', Carbon::now()->subMonth()->month)->whereYear('deactivation_date', Carbon::now()->subMonth()->year);
            });
        })->where('type',  'call')->whereNotNull('merchant')->get();
